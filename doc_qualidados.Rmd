...
A seguir, são carregados os pacotes do R (https://www.r-project.org) utilizados para filtragem e tratamento dos dados considerados no dashboard https://observatorioobstetrico.shinyapps.io/oobr_qualidados2/. Os dados do painel foram atualizados no dia 04/Junho/2023.
...
```{r include=FALSE}
df_total_max3 <- readRDS('data1/Sivep_2009-2022.rds ')
```
...
variaveis_ign <- c('CS_SEXO','CS_RACA','CS_ESCOL_N','CS_ZONA',
                ...
                'UTI','SUPORT_VEN','AMOSTRA','POS_PCRFLU','POS_PCROUT',
...
df_inconsistencia <- df_inconsistencia %>%
  mutate(condicao = case_when(
    ...
    df_gest_aux$UTI == '9') & (df_gest_aux$DT_ENTUTI != 'Em Branco') | (df_gest_aux$HOSPITAL == '2' | df_gest_aux$HOSPITAL == '9') & 
    df_gest_aux$UTI == '1'",
...
for (col in colnames(sivep)) {
  ...
  contagem_Impossivel <- sum(sivep[[col]] == "Impossivel", na.rm = TRUE)
  porc <- (contagem_Implausivel + contagem_Impossivel)/length(sivep[[col]])
  # Adição dos resultados à tabela
...
```{r include=FALSE}
load('data/SIM_Implau.rda')
load('data/SIM_Incom.rda')
load('data/SIM_Incon.rda')
```
